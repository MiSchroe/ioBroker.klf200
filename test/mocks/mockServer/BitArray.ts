 ...
 *
 * @param bitArray Bytes where each bit is set for the corresponding number, e.g. the node ID.
 * @returns Returns an array of numbers with an entry for each set bit.
 */
 ...
 *
 * @param numberArray Each number in the array corresponds to the bit that has to be set in the buffer.
 * @param bufferLength Length of the resulting buffer. This value will be ignored, if a destination buffer is provided.
 * @param [destinationBuffer] Instead of creating a new buffer, the result can be written directly to an existing buffer.
 * @returns Returns a new buffer with the bit array or the destination buffer, if a value for the destination buffer is provided.
 */
